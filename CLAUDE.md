# Claude Code Project Instructions — maplit-kotlin

## Project Overview

This is **maplit-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`maplit`](https://crates.io/crates/maplit).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/maplit/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/maplit>
